...
public class Main {
     ...
     */
    public static void giveRaise(List<Person> persons, String manager, String employee, double salary) throws NoSuchElementException, ClassCastException {
            ...
            if(p.getName().equals(employee)){
                ...
                if(p instanceof Employee){
                    for(Person a : persons){
                        if(a.getName().equals(manager)){
                            mfound = true;
                            if(a instanceof Manager){
                                ((Manager) a).giveRaise((Employee)p,salary);
                            }else {
        ...
        if(!efound) throw new NoSuchElementException(employee + " does not exist");
        if(!mfound) throw new NoSuchElementException(manager + " does not exist");
    }
    ...
    public static void assignPM(List<Person> persons, String developer, String manager) throws ClassCastException, NoSuchElementException, IllegalArgumentException{
                ...
                if(p instanceof Developer){
                    for(Person a: persons){
                        if(a.getName().equals(manager)){
                            mfound = true;
                            if(a instanceof Manager){
                                ((Developer) p).setProjectManager((Manager)a);
                            }else{
                                throw new ClassCastException(a.getName() + " is not a manager");
                            }
                        }
                    }
                }else{
        ...
        if(!dfound) throw new NoSuchElementException(developer + " does not exist");
        if(!mfound) throw new NoSuchElementException(manager + " does not exist");
    }
     ...
     */
    public static String customerSpeak(List<Person> persons, String customer, String employee) throws NoSuchElementException, ClassCastException{
                    ...
                    for(Person a : persons){
                        if(a.getName().equals(employee)){
                            ...
                            if(a instanceof Employee){
                                line = ((Customer) p).speak((Employee)a);
                            }else{
                                throw new ClassCastException(a.getName() + " is not an employee");
                            }
                        }
                    }
                }else{
        ...
        if(!cfound) throw new NoSuchElementException(customer + " does not exist");
        if(!efound) throw new NoSuchElementException(employee + " does not exist");
        return line;
    }
}
